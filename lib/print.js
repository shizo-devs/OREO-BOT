...
let _name = await conn.getName(m.sender) || 'Shizo'
...
let me = PhoneNumber('+' + (conn.user?.jid).replace('@s.whatsapp.net', '')).getNumber('international')
  ...
  console.log(`╭──»ShizoXbot«────✦ 
│Bot 🤖 : ${chalk.redBright('%s')}
│Sender✨: ${chalk.green('%s')}\n│${chalk.yellow('%s%s')}\n│Chat  📍: ${chalk.green('%s')}
╰────────────✦
${chalk.redBright.underline.bold('Message Caption is')}
`.trim(), 

            
me + ' ~' + conn.user.name + `${conn.user.jid == global.conn.user.jid ? '' : ' 🆂🅷🅸🆉🅾︎'}`,
